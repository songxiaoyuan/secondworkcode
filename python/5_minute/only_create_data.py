...
param_dic_au = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":1000,"file":file,"config_file":400}

param_dic_ag = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":15,"file":file,"config_file":410}

param_dic_j = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":100,"file":file,"config_file":420}

param_dic_jm = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":60,"file":file,"config_file":430}

param_dic_pp = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":5,"file":file,"config_file":440}

param_dic_v = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":5,"file":file,"config_file":450}

param_dic_y = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":10,"file":file,"config_file":460}

param_dic_p = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":10,"file":file,"config_file":470}

param_dic_c = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":10,"file":file,"config_file":480}

param_dic_a = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":10,"file":file,"config_file":490}

param_dic_m = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
			"multiple":10,"file":file,"config_file":500}

param_dic_jd = {"rsi_period":14,"limit_ema_tick_5":60,"limit_ema_tick_1":120,
...
class BandAndTrigger(object):
	...
	def __init__(self,param_dic):
		...
		self._limit_minute_hour_num =60
		self._current_minute_hour_num = 0

		self._limit_minute_5minute_num =5
		self._current_minute_5minute_num = 0

		self._file = param_dic["file"]
		self._config_file = param_dic["config_file"]

		if len(self._lastprice_array_hour) ==0:
			print "this is init function " + str(self._config_file)
			config_file = "../config/"+str(self._config_file)
			bf.get_config_info(self._lastprice_array_hour,self._lastprice_array_5minute,
					self._lastprice_array_1minute,config_file)
		# print self._pre_ema_val_60
		print len(self._lastprice_array_5minute)
		# print "the length of lastprice is: " +str(len(self._lastprice_array))

	def write_config(self):
		config_file = "../config/"+str(self._config_file)
		bf.write_config_info(self._lastprice_array_hour,self._lastprice_array_5minute,
				self._lastprice_array_1minute,self._ma_period,config_file,
				self._now_middle_60,self._now_middle_5,self._now_middle_1)
	...
	def get_md_data(self,md_array,lastone):
		...
		if lastone ==1:
			self._lastprice_array_hour.append(lastprice)
		...
		# self._now_middle_60 = bf.get_ema_data(lastprice,self._pre_ema_val_60,self._ma_period)
		self._now_middle_60 = bf.get_ma_data(lastprice,self._lastprice_array_hour,self._ma_period)
		...
		if minute != self._current_minute:
			self._current_minute_hour_num +=1
			self._current_minute_5minute_num +=1
			self._current_minute = minute
			self._lastprice_array_1minute.append(lastprice)

			if self._current_minute_hour_num > self._limit_minute_hour_num:
				self._current_minute_hour_num = 1
				self._lastprice_array_hour.append(lastprice)

			if self._current_minute_5minute_num > self._limit_minute_5minute_num:
				self._current_minute_5minute_num = 1
				self._lastprice_array_5minute.append(lastprice)
