...
def write_config_info(lastprice_array_hour,lastprice_array_5minute,lastprice_array_1minute,
	   ma_period,config_path,middle_value_60,middle_value_5,middle_value_1):
	config_file = open(config_path,"w")
	line1 = "lastprice_array_hour:"
	left = len(lastprice_array_hour)-ma_period
	if left<0:
		left = 0
	for i in xrange(left,len(lastprice_array_hour)):
		line1 = line1 + "," + str(lastprice_array_hour[i])
	line2 = "lastprice_array_5minute:"
	left = len(lastprice_array_5minute)-ma_period
	if left<0:
		left = 0
	for i in xrange(left,len(lastprice_array_5minute)):
		line2 = line2 + "," + str(lastprice_array_5minute[i])
	line3 = "lastprice_array_1minute:"
	left = len(lastprice_array_1minute)-ma_period
	if left<0:
		left = 0
	for i in xrange(left,len(lastprice_array_1minute)):
		line3 = line3 + "," + str(lastprice_array_1minute[i])
	line4 = "middle_value_60:,"+str(middle_value_60)
	line5 = "middle_value_5:,"+str(middle_value_5)
	line6 = "middle_value_1:,"+str(middle_value_1)
	write_lines = [line1+'\n',line2+'\n',line3]
	config_file.writelines(write_lines)
	config_file.close()

def get_config_info(lastprice_array_hour,lastprice_array_5minute,lastprice_array_1minute,config_path):
	try:
	...
	for line in lines:
		if "lastprice_array_hour" in line:
			...
			for tmp in line:
				tmp = float(tmp.strip())
				lastprice_array_5minute.append(tmp)
		elif "lastprice_array_1minute" in line:
			print "this is lastprice_array"
			line = line.split(',')[1:]
			for tmp in line:
				tmp = float(tmp.strip())
				lastprice_array_1minute.append(tmp)
